export default {
  async fetch(request, env) {
    ...
    const formType = formData.get("form_type"); // 'main', 'brittany', etc.
    ...
    // ---------------------------------------------------------
    // THE ROUTING MAP
    // This looks for Environment Variables in Cloudflare. 
    // If they don't exist yet, it falls back to a test email.
    // ---------------------------------------------------------
    const routeMap = {
      "main": env.EMAIL_MAIN || "",
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Website Forms <onboarding@resend.dev>", // Change this to their verified domain later
        to: destinationEmail,
